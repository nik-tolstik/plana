...
- Проект использует i18n и поддерживает языки `en` и `ru`; новые пользовательские runtime-тексты добавляй через переводы для обоих языков, а не хардкодь в компонентах.
...
- Общую инфраструктуру i18n, локали, ресурсы переводов и shared language UI держи в `src/shared/i18n`.
